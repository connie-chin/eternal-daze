...
import { viewShirtsProductDetail } from '../data';
...
export function ShirtDetails() {
  ...
  useEffect(() => {
    async function loadShirtProductDetails(productId: number) {
      setIsLoading(true);
      try {
        const productDetail = await viewShirtsProductDetail(productId);
        if (!productDetail)
    ...
    loadShirtProductDetails(Number(productId));
  }, [productId]);
