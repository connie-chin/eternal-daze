...
import { viewShoesProductDetail } from '../data';
...
export function ShoeDetails() {
  ...
  useEffect(() => {
    async function loadShoeProductDetails(productId: number) {
      setIsLoading(true);
      try {
        const productDetail = await viewShoesProductDetail(productId);
        if (!productDetail)
    ...
    loadShoeProductDetails(Number(productId));
  }, [productId]);
